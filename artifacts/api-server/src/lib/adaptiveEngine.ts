...
export const TradeOutcomeSchema = z.object({
  ...
  markPriceBeforeOrder: z.number().optional(),
  actualAvgEntryPrice: z.number().optional(),
  actualExitPrice: z.number().optional(),
  signalCreatedAt: z.number().optional(),
  qbEvaluatedAt: z.number().optional(),
  orderRequestedAt: z.number().optional(),
  orderSentAt: z.number().optional(),
  orderAckAt: z.number().optional(),
  positionConfirmedAt: z.number().optional(),
  positionClosedAt: z.number().optional(),
  protectionAttachedAt: z.number().optional(),
  monitorDetectedCloseAt: z.number().optional(),
  spreadBps: z.number().optional(),
  spreadAtSignal: z.number().optional(),
  spreadAtEntry: z.number().optional(),
  spreadAtExit: z.number().optional(),
  ...
  riskTier: z.enum(["MICRO", "SCOUT", "BASE", "BOOST", "AGGRESSIVE", "MAX_SNIPER"]).optional(),
  sizeMultiplier: z.number().positive().optional(),
  sizeReason: z.string().optional(),
  recommendedMargin: z.number().positive().optional(),
  recommendedLeverage: z.number().positive().optional(),
  maxLossIfStop: z.number().nonnegative().optional(),
  ...
  playbookScore: z.number().min(0).max(1).optional(),
  mlProbability: z.number().min(0).max(1).nullable().optional(),
  calibratedProbability: z.number().min(0).max(1).nullable().optional(),
  executionQuality: z.number().min(0).max(1).optional(),
  calibratedScore: z.number().min(0).max(1).optional(),
  readinessScopeId: z.string().optional(),
  promotionState: z.enum(["DEMO_ONLY", "SHADOW_LIVE", "MICRO_LIVE", "LIMITED_LIVE", "STANDARD_LIVE", "SUSPENDED"]).optional(),
